...
void main() {
    vec4 color = texture(uSampler, vec2(vTexCoord.s, vTexCoord.t)) * vec4(vColor.rgb, 1.0);
    if (color.a < 0.1) {
        discard;
    }
    outColor = vec4(color.rgb, vColor.a);
}
`;
...
class Renderer {
    ...
    constructor(id) {
        ...
        // Initialise WebGL2 (with WebGL1 fallback if needed)
    ...
    onBlockChanged(x, y, z) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (x >= c.start[0] && x < c.end[0] && y >= c.start[1] && y < c.end[1] && z >= c.start[2] && z < c.end[2]) {
                c.dirty = true;
            } else if (x >= c.start[0] && x < c.end[0] && y >= c.start[1] && y < c.end[1] && (z >= c.end[2] || z == c.start[2] - 1)) {
                c.dirty = true;
            } else if (x >= c.start[0] && x < c.end[0] && z >= c.start[2] && z < c.end[2] && (y == c.end[1] || y == c.start[1] - 1)) {
                c.dirty = true;
            } else if (y >= c.start[1] && y < c.end[1] && z >= c.start[2] && z < c.end[2] && (x == c.end[0] || x == c.start[0] - 1)) {
                c.dirty = true;
            }
        }
    }
...
// Keep global name for drop-in compatibility
